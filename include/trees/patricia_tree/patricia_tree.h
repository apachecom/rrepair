...
namespace trees {
    ...
    public:
         ...
         * */
        const C &operator[](const ulong &i) const{
            return dicc[i];
        }
        ...
        template<typename F>
        void dfs(const node *n, const F& f, const char &a, const ulong &j, const ulong& off,const ulong& prev_off) const {
            if (!f(n, a, j, off,prev_off))return;
            for (auto &&item :n->childs) {
                dfs(item.second.second, f, item.first, item.second.first, off + item.second.first,off);
            }
        }

        template<typename F,typename G>
        void dfs(const node *n, const F& f,const G& g, const char &a, const ulong &j) const {
            if (!f(n, a, j)) return;
...
//
//        template<typename F,typename G>
//        void dfs(const node *n, const F& f,const G& g, const char &a, const ulong &j, const ulong& off) const {
//
//            if (!f(n, a, j, off)) return;
//
//            for (auto &&item :n->childs) {
//                dfs(item.second.second, f, item.first, item.second.first, off + item.second.first );
//            }
//
//            if (!g(n, a, j,off)) return;
//        }
//
        ...
        /*
         * Provide bfs method over the tree
         * */
        template<typename F>
        void bfs(const node *n, const F& f, const char &a, const ulong &j) const {
            std::queue<std::pair<const node*,std::pair<char,ulong>>> Q;
            Q.push(std::make_pair(n,std::make_pair(a,j)));
            while(!Q.empty()){

                auto v = Q.front();
                Q.pop();

                const node* cnode = v.first;
                char cchar = v.second.first;
                ulong jj = v.second.second;

                if (!f(cnode, cchar, jj)) return;

                for (auto &&item :cnode->childs) {
                    Q.push(std::make_pair(item.second.second,std::make_pair(item.first,item.second.first)));
                }
            }
        }
