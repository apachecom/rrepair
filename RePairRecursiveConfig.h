...
//        const uint_t& ws,const uint_t& b,const uint_t& m,const std::string& input, const std::string& tempDir
    ...
    public:
        ...
        RePairRecursiveConfig(const ParseConf &pc, const DummyRepair &c, const uint_t &max_iter,
                              const uint_t &th_initial_seq) {
        ...
        RePairRecursiveConfig(
                const RePairRecursiveConfig<uint_t_64, DummyRepair, HashParser<HashParserConfig<KRPSlindingWindow<>, KRPHashFunction<uint_t_64, std::string> > >> &RPC) {
        ...
        RePairRecursiveConfig &operator=(
                const RePairRecursiveConfig<uint_t_64, DummyRepair, HashParser<HashParserConfig<KRPSlindingWindow<>, KRPHashFunction<uint_t_64, std::string> > >> &RPC) {
...
//                tmp_files.clear();
        ...
        uint_t_64 setIter(const uint_t_64 &i) {
            _iter = i;
            return _iter;
        }
        ...
        uint_t_64 setSizeSeq(const uint_t_64 &s) {
            _size_seq = s;
            return _size_seq;
        }
        ...
        * */
        ...
        std::pair<uint_t, uint_t> postprocess() {
            ...
            for (uint_t_64 i = 0; i < repairFilesR.size() - 1; ++i) {
                ...
                postprocessDiccCFile(fileC, R, n_rules_level, offset_rules, maxSigmaIt[i].first, maxSigmaIt[i].second,
                                     rule_map);
            ...
            * */
            ...
            std::pair<uint_t_64, uint_t_64> p = posprocessingFinalParseFile(R, C, offset_rules, initial_sigma,
                                                                            rule_map);
    ...
    protected:
        ...
        */
        uint_t_64 postprocessDiccRFile(std::fstream &file, std::fstream &R, const uint_t_64 &offset_rules,
                                       const uint_t_64 &min_sigma, std::map<uint_t_64, uint_t_64> &rule_map) {
...
//                }
         ...
         * */
        void postprocessDiccCFile(std::fstream &file, std::fstream &R, uint_t_64 &n_rules_level, const uint_t_64 &offset_rules,
                                  const uint_t_64 &min_sigma,const uint_t_64 &max_sigma,std::map<uint_t_64, uint_t_64> &rule_map
        ) {
...
//            std::cout<<"Parser sequence:"<<std::endl;
        ...
        std::pair<uint_t_64, uint_t_64> posprocessingFinalParseFile(std::fstream &R,std::fstream &C,uint_t_64 &offset_rules,const uint_t_64 &initial_sigma,std::map<uint_t_64, uint_t_64> &rule_map
        ) {
...

            {
                ...
                /**reading rules*/
                int XX = 0, YY = 0;
                while (!r_file.eof() && r_file.read((char *) &XX, sizeof(int)) &&
